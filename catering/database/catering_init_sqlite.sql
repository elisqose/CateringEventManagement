...
DROP TABLE IF EXISTS `PersonnelAvailability`;

DROP TABLE IF EXISTS `ProposalItems`;

DROP TABLE IF EXISTS `ProposalsToModify`;

DROP TABLE IF EXISTS `ServiceAssignments`;
...
DROP TABLE IF EXISTS `RecurringEvents`;
...
CREATE TABLE
    `RecurringEvents` (
        ...
        `numero_servizi_richiesti` INTEGER DEFAULT 0,
        `numero_partecipanti` INTEGER DEFAULT 0,
        `cliente` TEXT,
        `note` TEXT,
        `organizzatore_id` INTEGER,
        `tipo_frequenza` TEXT NOT NULL DEFAULT 'WEEKLY',
        `data_fine` DATE,
...
CREATE TABLE
    `Events` (
        ...
        `numero_servizi_richiesti` INTEGER DEFAULT 0,
        `numero_partecipanti` INTEGER DEFAULT 0,
        `num_partecipanti_iniziale` INTEGER DEFAULT 0,
        `cliente` TEXT,
        `note` TEXT,
        `documentazione_finale` TEXT,
        `stato` TEXT NOT NULL DEFAULT 'BOZZA',
        `chef_id` INTEGER,
        `organizzatore_id` INTEGER,
        `macro_evento_id` INTEGER,
        FOREIGN KEY (`chef_id`) REFERENCES `Users` (`id`),
        FOREIGN KEY (`organizzatore_id`) REFERENCES `Users` (`id`),
        FOREIGN KEY (`macro_evento_id`) REFERENCES `RecurringEvents` (`id`)
    );
...
CREATE TABLE
    `Services` (
        ...
        `name` TEXT,
        `tipologia` TEXT,
        `offset_inizio` INTEGER DEFAULT 0,
        `ora_inizio` TIME,
        `ora_fine` TIME,
        `numero_commensali` INTEGER DEFAULT 0,
...
CREATE TABLE
    `ServiceShifts` (
        ...
        `fine` DATETIME,
        `luogo` TEXT,
        `offset_prima` INTEGER DEFAULT 0,
        `offset_dopo` INTEGER DEFAULT 0,
        `cuoco_supporto_id` INTEGER,
        FOREIGN KEY (`service_id`) REFERENCES `Services` (`id`),
        FOREIGN KEY (`cuoco_supporto_id`) REFERENCES `Users` (`id`)
    );
...
CREATE TABLE
    `ServiceAssignments` (
        ...
        `ruolo` TEXT,
        FOREIGN KEY (`turno_id`) REFERENCES `ServiceShifts` (`id`),
        FOREIGN KEY (`personale_id`) REFERENCES `Users` (`id`)
    );
...
CREATE TABLE
    `ProposalItems` (
        ...
        PRIMARY KEY (`proposal_id`, `menu_item_id`),
        FOREIGN KEY (`proposal_id`) REFERENCES `ProposalsToModify` (`id`),
...
-- Create a new event (organizzatore: Giovanni id 7, chef: Antonio id 5)
INSERT INTO Events (
    name, luogo, data_inizio, durata, numero_servizi_richiesti,
    numero_partecipanti, num_partecipanti_iniziale, cliente, note,
    stato, chef_id, organizzatore_id
) VALUES (
    'Gala Aziendale Annuale', 'Villa Reale', date('2025-06-15'), 2, 2,
    120, 120, 'Acme Corp', '',
    'IN_CORSO', 5, 7
);
...
INSERT INTO Services (
    ...
    'Pranzo',                   -- Tipologia
    0,                          -- Same day as event start
    time('12:00'),              -- Start time
    time('15:00'),              -- End time
    120,                        -- Numero commensali
    'CONFERMATO',                -- Menu already approved
    'Salone Grande',            -- Location in Italian
    1                           -- Chosen menu (same as proposed)
);
...
INSERT INTO Services (
    ...
    time('23:00'),              -- End time
    80,                         -- Numero commensali
    'IN_PREPARAZIONE',          -- Not confirmed yet
    'Sala Esecutiva',           -- Location in Italian
    0                           -- No menu chosen yet
);

-- Service shift for the lunch service, with support cook Sofia (id 4)
INSERT INTO ServiceShifts (
    service_id, inizio, fine, luogo, offset_prima, offset_dopo, cuoco_supporto_id
) VALUES (
    1, '2025-06-15 11:00:00', '2025-06-15 16:00:00', 'Villa Reale', 60, 60, 4
);

-- Staff assigned to that shift (Marco, wait staff)
INSERT INTO ServiceAssignments (turno_id, personale_id, ruolo) VALUES
(1, 1, 'Cameriere');

-- Availability of staff/cooks around the sample event's dates
INSERT INTO PersonnelAvailability (user_id, inizio, fine) VALUES
(1, '2025-06-15 10:00:00', '2025-06-15 17:00:00'),  -- Marco (staff)
(2, '2025-06-15 17:00:00', '2025-06-15 23:59:00'),  -- Giulia (staff)
(3, '2025-06-15 10:00:00', '2025-06-15 23:59:00'),  -- Luca (cook)
(4, '2025-06-15 10:00:00', '2025-06-15 17:00:00');  -- Sofia (cook)