...
public class MenuItem {
    ...
    /**
     * Loads a single item by id, regardless of which menu or section it
     * belongs to. Used to rebuild the voci of a Proposta di Modifica, which
     * references items by id through ProposalItems.
     */
    public static MenuItem loadById(int id) {
        ...
        PersistenceManager.executeQuery(query, new ResultHandler() {
            ...
            public void handle(ResultSet rs) throws SQLException {
                ...
                mi.description = rs.getString("description");
                holder[0] = mi;
                recid[0] = rs.getInt("recipe_id");
            }
        }, id);

        if (holder[0] != null) {
            holder[0].recipe = Recipe.loadRecipe(recid[0]);
        }
        return holder[0];
    }
